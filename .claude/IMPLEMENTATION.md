...
| 4 | FSE structure                  | 🟡    | `style.css` header complete, `theme.json` has `$schema`, `functions.php` require-only. ABSPATH guards now present in all 14 `inc/*.php`. Still missing: `patterns/`, `languages/` (.pot), `$schema` in `block.json`. |
...
- [ ] **3. JS/CSS tooling** — `_dev/.eslintrc.json`, `.stylelintrc.json`, `.prettierrc`, `.prettierignore`; update `_dev/package.json` (deps + scripts). Doc updates: `.claude/CONVENTIONS.md` CSS + JS sections (remove ⚠️ flags), `.claude/ARCHITECTURE.md` `_dev/` tree.
...
- [ ] **5. Prefix + namespace migration** — `studio_` → `sv_boilerplate_`; `StudioVal\WPBoilerplate\` → `StudioVal\Boilerplate\`; remaining `theme-name` placeholders in `_dev/scripts/make-block.js` (block namespace, category, CSS class) + block namespaces in `_dev/blocks/`, `composer.json` autoload, `bin/setup.sh` substitutions, `.github/copilot-instructions.md`. Doc updates: `.claude/CLAUDE.md` (Code conventions Hook prefix + PHP namespace + Known pitfalls), `.claude/CONVENTIONS.md` (PHP function prefix + PHP namespace sections).
