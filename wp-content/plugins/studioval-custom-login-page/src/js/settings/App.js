...
export default function App() {
  ...
  const addNotice = useCallback((content, status = "success") => {
    setNotices((prev) => [
      ...prev,
      { id: String(Date.now()), content, status },
    ]);
  }, []);
  ...
  const resetKeys = useCallback(
    (keys) => {
      ...
      addNotice(
        __("Defaults restored — click Save to apply.", "studioval-clp"),
      );
    },
    [addNotice],
  );
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      addNotice(
        error?.message ||
          __("An error occurred while saving.", "studioval-clp"),
        "error",
      );
    } finally {
      setIsSaving(false);
  ...
  const hasImageLayout = ["image-left", "image-right"].includes(
    settings.layout,
  );
    ...
    <div className="clp-settings-wrap">
      <VStack spacing={5}>
        <HStack justify="space-between" align="center">
          ...
          >
            {isSaving
              ? __("Saving…", "studioval-clp")
              : __("Save", "studioval-clp")}
          </Button>
        </HStack>
